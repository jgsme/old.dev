...
module.exports = (dest)->
  ...
  flush = (callback)->
    ...
    , (err, posts)->
      ...
      allWrite = (archive, cb)-> fs.writeFile do
        path.resolve dest, "page-#{archive.index}.json"
        JSON.stringify archive
        (err)-> if err? then cb err else cb null
      yearWrite = (key, cb)-> fs.writeFile do
        path.resolve dest, "#{key}.json"
        JSON.stringify yearlyArchive[key]
        (err)-> if err? then cb err else cb null
      monthWrite = (key, cb)-> fs.writeFile do
        path.resolve dest, "#{key}.json"
        JSON.stringify monthlyArchive[key]
        (err)-> if err? then cb err else cb null
